...
load('./datasets/nci9.mat')
load('datasets\pcmac_weight.mat')
fea = X;
gnd = Y;
...
d = 9; %%% dimension of subspaces

%% choose the running algorithms
run_KS = 0; run_SC = 0; run_TS = 1; run_GS = 0; run_LR = 0; run_LC = 0; run_MP = 0; 

%% run the algorithms
repeat_num = 5; max_iternum = 2e2;
...
for repeat = 1:repeat_num
    ...
    if run_KS == 1
        tau = 9.9e-1; SC_type = 1; %% 0 = normalized spectral clustering
        init = 0; %% 1 = random initialization; 0 = initialization by TIPS
        opts = struct('tau', tau, 'iternum', max_iternum, 'tol', 1e-3, 'print', 0, 'init', init, 'SC_type', SC_type);
