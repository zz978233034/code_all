...
load('./datasets/text1_uni.mat')
% X = fea;
% Y = gnd;
Z = X'; true_labels = Y; %%% Z: data matrix; labels: ground truth;
if min(Y) == 0
    true_labels = Y + 1;
end
...
%% parameter settings
K = max(Y); %%% number of subspaces
[n, N] = size(Z); %%% N: number of samples
d = 8; %%% dimension of subspaces

%% choose the running algorithms
run_KS = 1; run_SC = 1; run_TS = 1; run_GS = 1; run_LR = 1; run_LC = 0; run_MP = 0;

%% run the algorithms
repeat_num = 3; max_iternum = 2e2;
